...
const GroupController = {
   ...
   */
	addEvents: async (req, res) => {
		...
		try {
			...
			await group.save();
			return res.json(
				await Event.find({ _id: { $in: group.events }})
			);
		} catch (e) {
			return res.status(400).json({ message: 'DB Error', e });
		}
   ...
   */
	...
	removeEvents: async (req, res) => {
		...
		try {
			...
			events.forEach(event => {
				if (group.events.includes(event))
					removedEvents.push(...group.events.splice(group.events.indexOf(event), 1));
			});
			await group.save();
   ...
   */
	getParticipants: async (req, res) => {
		...
		try {
			const group = await Group.findById(id, { participants: 1 }).populate('participants');
			if (!group)
				return res.status(404).json({ message: 'No such group exists'});
	...
	removeParticipants: async (req, res) => {
		...
		try {
			...
			participants.forEach(participant => {
				if (group.participants.includes(participant))
					removedParticipants.push(...group.participants.splice(group.participants.indexOf(participant), 1));
			});
			await group.save();
